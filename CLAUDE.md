...
### Mode privé
- Auto : items marqués sensibles jamais stockés — `org.nspasteboard.ConcealedType`,
  `TransientType`, `AutoGeneratedType`, `com.apple.is-sensitive`.
- Manuel : toggle « Pause » dans le panneau → suspend la capture (pour saisies sensibles).
- **Limite connue** : un gestionnaire de mots de passe **via extension navigateur**
  (ex. LastPass dans Safari) copie du **texte brut sans marqueur** → indétectable
  automatiquement. Parade : la **pause manuelle**. (Décision : pas d'heuristique
  « ressemble à un mot de passe » — trop de faux positifs.)

### Recherche
- Hybride : correspondances littérales (sous-chaîne) puis sémantiques (`NLContextualEmbedding`, cosinus).
...
2. `ClipboardStore` ignore la capture si la pause manuelle est active ;
   `PasteboardReader` rejette les types *concealed*/*transient* et renvoie un `CapturedContent`.
