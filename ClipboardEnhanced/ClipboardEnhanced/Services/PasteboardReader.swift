...
struct PasteboardReader {
    ...
    /// Marqueurs de contenu sensible posés par les gestionnaires de mots de passe.
    /// Convention nspasteboard.org + variantes connues.
    private static let sensitiveTypes: Set<String> = [
        "org.nspasteboard.ConcealedType",       // 1Password, standard
        "org.nspasteboard.TransientType",       // ne pas historiser
        "org.nspasteboard.AutoGeneratedType",   // généré automatiquement
        "com.apple.is-sensitive",               // marqueur sensible Apple
    ]
    ...
    func read() -> CapturedContent? {
        ...
        // (gestionnaires natifs). NB : une extension navigateur (LastPass/Safari)
        // copie du texte brut SANS marqueur → indétectable ; utiliser la pause manuelle.
        if isSensitive(types) {
            return nil
        }
    ...
    private func isSensitive(_ types: [NSPasteboard.PasteboardType]) -> Bool {
        types.contains { Self.sensitiveTypes.contains($0.rawValue) }
    }
